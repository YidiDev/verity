...
_transport_mode = os.environ.get("TELEHEALTH_BASELINE_TRANSPORT", "sse").strip().lower()
if _transport_mode not in {"sse", "polling"}:
    _transport_mode = "sse"

try:
    _poll_interval = int(os.environ.get("TELEHEALTH_BASELINE_POLL_MS", "10000"))
except ValueError:
    _poll_interval = 10000

_poll_interval = max(1000, _poll_interval)

app.config.update(
    BASELINE_TRANSPORT_MODE=_transport_mode,
    BASELINE_POLL_INTERVAL_MS=_poll_interval,
)
...
def home() -> str:
    return render_template(
        "index.html",
        transport_mode=app.config.get("BASELINE_TRANSPORT_MODE", "sse"),
        poll_interval_ms=app.config.get("BASELINE_POLL_INTERVAL_MS", 10000),
    )
