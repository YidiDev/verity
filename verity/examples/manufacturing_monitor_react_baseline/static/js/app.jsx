...
const COLLECTION_CONFIG = {
  recent_defects: { limit: 60, sortKey: "detected_at" },
  downtime_events: { limit: 60, sortKey: "started_at" },
  quality_audits: { limit: 150, sortKey: "logged_at" },
  maintenance_backlog: { limit: 150, sortKey: "due" },
  shift_notes: { limit: 150, sortKey: "logged_at" },
  supply_runs: { limit: 120, sortKey: "logged_at" },
  outbound_shipments: { limit: 120, sortKey: "logged_at" },
  inventory_positions: { limit: 150, sortKey: "last_updated" },
  safety_incidents: { limit: 120, sortKey: "logged_at" },
  safety_walks: { limit: 120, sortKey: "logged_at" },
};

const COLLECTION_KEYS = Object.keys(COLLECTION_CONFIG);
...
function mergeEntities(existing, incoming) {
  ...
  for (const key of Object.keys(incoming)) {
    base[key] = deepClone(incoming[key]);
  }
  return base;
}
...
function normalizeCollection(value, key) {
  ...
  if (!value || typeof value !== "object") {
    const pageSize = Math.min(config.limit || 25, 25) || 1;
    return {
      items: [],
      page: 1,
      page_size: pageSize,
  ...
  const items = Array.isArray(value.items)
    ? value.items.map((entry) => mergeEntities(null, entry))
    : [];
  const pageSize =
    value.page_size || Math.min(items.length || config.limit || 1, config.limit || items.length || 1) || 1;
  const total = value.total ?? items.length;
  const totalPages = value.total_pages || Math.max(1, Math.ceil((total || items.length) / (pageSize || 1)));
  return {
    ...value,
    items,
    page: value.page || 1,
...
function normalizeDashboard(payload) {
  ...
  normalized.lines = Array.isArray(payload.lines)
    ? payload.lines.map((line) => mergeEntities(null, line))
    : [];
  normalized.viewer = payload.viewer ? mergeEntities(null, payload.viewer) : null;
  normalized.quality_summary = payload.quality_summary
    ? mergeEntities(null, payload.quality_summary)
    : null;
  normalized.training_compliance = payload.training_compliance
    ? mergeEntities(null, payload.training_compliance)
    : null;
  normalized.inventory_catalog = Array.isArray(payload.inventory_catalog)
    ? [...payload.inventory_catalog]
    : [];
  COLLECTION_KEYS.forEach((key) => {
    normalized[key] = normalizeCollection(payload[key], key);
  });
  return normalized;
}

function chooseLatest(existing, incoming, sortKey) {
  if (!existing) {
    return mergeEntities(null, incoming);
  }
  if (!incoming) {
    return existing;
  }
  ...
  for (const key of candidates) {
    const incomingTs = parseTimestamp(incoming[key]);
    const existingTs = parseTimestamp(existing[key]);
...
function upsertLines(lines, incoming) {
  if (!incoming || !incoming.id) {
    return { changed: false, lines };
  }
  const safeLines = Array.isArray(lines) ? lines.slice() : [];
  const index = safeLines.findIndex((entry) => entry.id === incoming.id);
  let changed = false;
  if (index >= 0) {
    const merged = chooseLatest(safeLines[index], incoming, "last_updated");
    if (!entitiesEqual(merged, safeLines[index])) {
      safeLines[index] = merged;
      changed = true;
    }
  } else {
    safeLines.push(mergeEntities(null, incoming));
    changed = true;
  }
  return { changed, lines: changed ? safeLines : lines };
}

function mergeCollectionItem(collection, item, key) {
  if (!item) {
    return { changed: false, collection };
  }
  const config = COLLECTION_CONFIG[key] || {};
  const base = collection && typeof collection === "object" ? collection : normalizeCollection(null, key);
  const items = Array.isArray(base.items) ? base.items.slice() : [];
  const index = item.id ? items.findIndex((entry) => entry.id === item.id) : -1;
  let mutated = false;
  if (index >= 0) {
    const merged = chooseLatest(items[index], item, config.sortKey);
    if (!entitiesEqual(merged, items[index])) {
      items[index] = merged;
      mutated = true;
    }
  } else {
    items.push(mergeEntities(null, item));
    mutated = true;
  }
  if (config.sortKey) {
    items.sort((a, b) => {
      const left = parseTimestamp(a[config.sortKey]) || 0;
      const right = parseTimestamp(b[config.sortKey]) || 0;
      return right - left;
    });
  }
  ...
  const changed =
    mutated ||
    trimmed.length !== (base.items ? base.items.length : 0) ||
    trimmed.some((entry, idx) => !entitiesEqual(entry, base.items ? base.items[idx] : null));
  if (!changed) {
    return { changed: false, collection: base };
  }
  const pageSize = base.page_size || Math.min(trimmed.length || 1, limit || trimmed.length || 1) || 1;
  const total = Math.max(base.total || 0, trimmed.length);
...
function replaceCollectionWithSnapshot(collection, snapshot, key) {
  const config = COLLECTION_CONFIG[key] || {};
  const base = collection && typeof collection === "object" ? collection : normalizeCollection(null, key);
  const items = Array.isArray(snapshot) ? snapshot.map((entry) => mergeEntities(null, entry)) : [];
  if (config.sortKey) {
    items.sort((a, b) => {
      const left = parseTimestamp(a[config.sortKey]) || 0;
      const right = parseTimestamp(b[config.sortKey]) || 0;
...
function buildCatalogFromCollection(collection) {
...
function reconcileEventState(data, payload) {
  if (!payload || typeof payload !== "object") {
  ...
  const next = { ...data };
  if (payload.line) {
    const result = upsertLines(data.lines, payload.line);
    if (result.changed) {
      next.lines = result.lines;
      changed = true;
    }
  }
  switch (payload.type) {
    case "downtime_event": {
      const result = mergeCollectionItem(data.downtime_events, payload.event, "downtime_events");
      if (result.changed) {
        next.downtime_events = result.collection;
        changed = true;
      }
      break;
    }
    ...
    case "quality_alert": {
      const result = mergeCollectionItem(data.recent_defects, payload.defect, "recent_defects");
      if (result.changed) {
        next.recent_defects = result.collection;
        changed = true;
      }
      break;
    }
    case "quality_audit": {
      if (payload.summary) {
        next.quality_summary = mergeEntities(data.quality_summary || {}, payload.summary);
        changed = true;
      }
      const result = mergeCollectionItem(data.quality_audits, payload.audit, "quality_audits");
      if (result.changed) {
        next.quality_audits = result.collection;
        changed = true;
      }
      break;
    }
    ...
    case "shift_note": {
      const result = mergeCollectionItem(data.shift_notes, payload.note, "shift_notes");
      if (result.changed) {
        next.shift_notes = result.collection;
        changed = true;
      }
      break;
    }
    case "supply_run": {
      const runResult = mergeCollectionItem(data.supply_runs, payload.delivery, "supply_runs");
      if (runResult.changed) {
        next.supply_runs = runResult.collection;
        changed = true;
      }
      if (Array.isArray(payload.inventory)) {
        const inventoryResult = replaceCollectionWithSnapshot(
          next.inventory_positions,
          payload.inventory,
          "inventory_positions",
        );
        if (inventoryResult.changed) {
          next.inventory_positions = inventoryResult.collection;
          changed = true;
        }
        next.inventory_catalog = buildCatalogFromCollection(next.inventory_positions);
      }
      break;
    }
    case "inventory_adjustment": {
      if (payload.inventory_item) {
        const inventoryItemResult = mergeCollectionItem(
          next.inventory_positions,
          payload.inventory_item,
          "inventory_positions",
        );
        if (inventoryItemResult.changed) {
          next.inventory_positions = inventoryItemResult.collection;
          changed = true;
        }
      }
      if (Array.isArray(payload.inventory)) {
    ...
    case "shipment_update": {
      const result = mergeCollectionItem(data.outbound_shipments, payload.shipment, "outbound_shipments");
      if (result.changed) {
        next.outbound_shipments = result.collection;
        changed = true;
      }
      break;
    }
    case "safety_incident": {
      if (payload.incident) {
        const incidentResult = mergeCollectionItem(data.safety_incidents, payload.incident, "safety_incidents");
        if (incidentResult.changed) {
          next.safety_incidents = incidentResult.collection;
          changed = true;
        }
      }
      if (payload.training) {
        next.training_compliance = mergeEntities(data.training_compliance || {}, payload.training);
        changed = true;
      }
      break;
    }
    case "safety_walk": {
      if (payload.walk) {
        const walkResult = mergeCollectionItem(data.safety_walks, payload.walk, "safety_walks");
        if (walkResult.changed) {
          next.safety_walks = walkResult.collection;
          changed = true;
        }
      }
      if (payload.training) {
  ...
  return { changed, data: changed ? next : data };
}
...
function useDashboard(userId) {
  ...
  const fetchDashboard = useCallback(async () => {
    if (!userId) {
      updateState(createInitialDashboardState());
      return { ok: false };
    }
  ...
  }, [fetchDashboard, updateState, userId]);
  ...
  const applyEvent = useCallback(
    (payload) => {
      ...
      if (payload.type === "ready") {
        updateState((prev) => ({ ...prev, lastEvent: payload }));
        return { handled: true };
      }
      ...
      const result = reconcileEventState(current.data, payload);
      if (!result.changed) {
        updateState((prev) => ({ ...prev, lastEvent: payload }));
        return { handled: false };
      }
...
function useLines(data) {
  return useMemo(() => (Array.isArray(data?.lines) ? data.lines : []), [data]);
}

function useCollectionResource(data, key) {
  return useMemo(() => {
...
function useRecentDefects(data) {
  return useCollectionResource(data, "recent_defects");
}

function useDowntimeEvents(data) {
  return useCollectionResource(data, "downtime_events");
}

function useQualityAudits(data) {
  return useCollectionResource(data, "quality_audits");
}

function useMaintenanceBacklog(data) {
  return useCollectionResource(data, "maintenance_backlog");
}

function useShiftNotes(data) {
  return useCollectionResource(data, "shift_notes");
}

function useSupplyRuns(data) {
  return useCollectionResource(data, "supply_runs");
}

function useInventoryPositions(data) {
  return useCollectionResource(data, "inventory_positions");
}

function useOutboundShipments(data) {
  return useCollectionResource(data, "outbound_shipments");
}

function useSafetyIncidents(data) {
  return useCollectionResource(data, "safety_incidents");
}

function useSafetyWalks(data) {
  return useCollectionResource(data, "safety_walks");
}

function useQualitySummary(data) {
  return useMemo(() => (data?.quality_summary ? { ...data.quality_summary } : null), [data]);
}

function useTrainingCompliance(data) {
  return useMemo(() => (data?.training_compliance ? { ...data.training_compliance } : null), [data]);
}

function useInventoryCatalog(data) {
  return useMemo(() => (Array.isArray(data?.inventory_catalog) ? data.inventory_catalog : []), [data]);
}
...
function OverviewPage({ data, selectedLineId, onSelectLine, refresh, onNotify }) {
  ...
  const inventoryPositions = useInventoryPositions(data);
  const inventoryItems = inventoryPositions.items || [];
  const recentDefects = useRecentDefects(data);
  const downtimeResource = useDowntimeEvents(data);
  const selectedLine =
      ...
      <Section title="Factory activity" description="Latest quality and maintenance signals for this viewer.">
        <div className="grid gap-6 lg:grid-cols-2">
          <div>
            ...
            <RecentDefects defects={recentDefects} />
          </div>
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-400">
              Downtime timeline
            </h3>
            <DowntimeEvents events={downtimeResource} />
          </div>
        </div>
      </Section>
    </div>
...
function QualityPage({ data, refresh, onNotify, defaultLineId }) {
  const lines = useLines(data);
  const viewer = data.viewer || {};
  const qualitySummary = useQualitySummary(data);
  const qualityAuditsResource = useQualityAudits(data);
  const qualityAudits = qualityAuditsResource.items || [];
  const recentDefects = useRecentDefects(data);
...
function MaintenancePage({ data, refresh, onNotify }) {
  const lines = useLines(data);
  const downtimeResource = useDowntimeEvents(data);
  const downtimeEvents = downtimeResource.items || [];
  const backlogResource = useMaintenanceBacklog(data);
  const backlog = backlogResource.items || [];
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <MaintenanceBacklog items={backlogResource} />
      </Section>
    </div>
  );
}
...
function LogisticsPage({ data, refresh, onNotify, defaultLineId }) {
  const lines = useLines(data);
  const inventoryCatalog = useInventoryCatalog(data);
  const shipmentsResource = useOutboundShipments(data);
  const shipments = shipmentsResource.items || [];
  const inventoryResource = useInventoryPositions(data);
  const supplyResource = useSupplyRuns(data);
...
function SafetyPage({ data, refresh, onNotify, defaultLineId }) {
  const lines = useLines(data);
  const incidentsResource = useSafetyIncidents(data);
  const incidents = incidentsResource.items || [];
  const walksResource = useSafetyWalks(data);
  const training = useTrainingCompliance(data);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <SafetyBoard incidents={incidentsResource} walks={walksResource} training={training} />
      </Section>
...
function HandoverPage({ data, refresh, onNotify }) {
  const viewer = data.viewer || {};
  const shiftNotesResource = useShiftNotes(data);
  ...
  return (
    <div className="space-y-6">
      ...
      <Section title="Shift log" description="Supervisor hand-off log">
        <ShiftNotes notes={shiftNotesResource} />
      </Section>
    </div>
  );
}
...
function App() {
  ...
  const { loading, error, data, refresh, applyEvent } = useDashboard(selectedUserId);
  ...
  const handleEvent = useCallback(
    (payload) => {
      ...
      const result = applyEvent(payload);
      if (!result?.handled && payload?.type !== "ready") {
        refresh();
      }
    },
    [applyEvent, refresh, showToast],
  );
