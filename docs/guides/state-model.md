...
### After Failure: Settles Until Explicit Retry

Failed fetches remain as an honest error state instead of being restarted by every component render. Ordinary reads do not retry a failed request. Retry from an explicit user action with `{ force: true }`, or let a server refresh directive request the retry.
...
const user = DL.fetchItem('user', 123)
// After rejection: user.meta.error contains the error string.
// Re-reading does not issue another request.
...
A successful fetch or inline directive result clears the corresponding failure marker. Item failures and errors are tracked per level, so unrelated level success does not hide an unresolved error. Parameterized collections are tracked per parameter set.
